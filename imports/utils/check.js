...
const testSubtree = (value, pattern) => {
  ...
  for (let i = 0; i < typeofChecks.length; ++i) {
    if (pattern === typeofChecks[i][0]) {
      ...
      return {
        message: `Expected ${
          typeofChecks[i][1]
        }, got ${stringForErrorMessage(value, { onlyShowType: true })}`,
        path: ""
      };
    }
  }
  ...
  if (pattern instanceof Array) {
    if (pattern.length !== 1) {
      return {
        message: `Bad pattern: arrays must have one type element ${stringForErrorMessage(
          pattern
        )}`,
        path: ""
      };
...
class ArgumentChecker {
  ...
  throwUnlessAllArgumentsHaveBeenChecked() {
    if (this.args.length > 0)
      throw new Error(
        `Did not check() all arguments during ${this.description}`
      );
  }
}
