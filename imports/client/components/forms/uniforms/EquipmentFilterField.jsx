...
export const GET_SETTINGS_DATA = gql`
  query getAccountSettingsEquipmentFilterField {
...
const quantityDropdown = ({ disabled, id, inputRef, error, options, onChange, value = "" }) => {
  ...
  const transportUnits = opts.filter(({ type }) => type === "TU");
  ...
  return (
    ...
    >
      <Dropdown.Menu>
        ...
        <Dropdown.Divider />
        <Dropdown.Header
          icon={ITEM_TYPE_ICONS.TU}
          content={t("shipment.form.item.quantityType.TU")}
        />
        <Dropdown.Menu scrolling>
          {transportUnits.map((option, i) => (
            <Dropdown.Item
              key={i}
