...
export class PriceListGridSheet2 extends React.Component {
  ...
  handleOnChange = updates => {
    const { onSave, priceListComp } = this.props;
    const changes = updates.map(
      ({ rowPinned, rowIndex, colDef, node: rowNode, oldValue, newValue }) => {
        const { field } = colDef;
        const row = rowPinned ? rowIndex : this.pinnedTopRowData.length + rowIndex;
    ...
    const changesData = priceListComp.parseChanges(changes); // returns { priceListId, updates }
    saveRates.call(priceListComp, changesData);
  };

  /**
   * on delete range of cells of grid, return true will make cells empty
   */
  async onDelete(updates){
    const { onSave, priceListComp } = this.props;
    
    const changes = updates.map(
      ({ rowPinned, rowIndex, colDef, oldValue }) => {
        const { field } = colDef;
        const row = rowPinned ? rowIndex : this.pinnedTopRowData.length + rowIndex;

        // it exclude the rowHeader
        const col = parseInt(field) - 1;
        return [row, col, oldValue, null];
      }
    );
  ...
  getCellMeta({ rowIndex, field, rowPinned }) {
    const { priceListComp } = this.props;
    const { base, gridcols, gridrows } = priceListComp;
    ...
    if (rowPinned !== "top") {
      dataRowIndex += this.pinnedTopRowData.length;

      // normal cell
      const { editor, type } = this.columnsUIMeta[dataColIndex];
    ...
    // find format
    const foundFormat = this.cellFormatAndDropdowns.find(({ row, col }) => {
  ...
  buildMenuItem({ selection, item, rowIndex, cellIndex, rowPinned }) {
    ...
    const pinnedRowCount = this.pinnedTopRowData.length;
  ...
  convertDataStructure(priceListComp) {
    const { base: oldData, gridcols, gridrows, data: oldData2 } = priceListComp;

    const { rowHeaders, colHeaders, columnsUIMeta } = {
      rowHeaders: priceListComp.getHeaders("rows"),
      colHeaders: priceListComp.getHeaders("cols"),
      columnsUIMeta: priceListComp.getColumns()
    };

    const cellFormatAndDropdowns = priceListComp.setCellFormatAndDropdowns();
  ...
  render() {
    const { style, priceListComp, t } = this.props;
    ...
    const {
      ...
      // this meta is for the normal column cell
      columnsUIMeta
    } = this.convertDataStructure(priceListComp);
    this.columnsUIMeta = columnsUIMeta;
    this.cellFormatAndDropdowns = cellFormatAndDropdowns;
    this.contextMenuComp = getGridContextMenuComp(priceListComp, this.t).get();
